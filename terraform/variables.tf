...
variable "gpu_node_instance_type" {
  ...
  validation {
    condition     = length(var.gpu_node_instance_type) > 0
...
variable "personaplex_service_accounts" {
  description = "List of namespace:serviceaccount for the personaplex IRSA role"
  type        = list(string)
...
variable "voice_generator_service_accounts" {
  description = "List of namespace:serviceaccount for the voice-generator IRSA role"
  type        = list(string)
  default     = ["personaplex:voice-generator-sa"]
}
