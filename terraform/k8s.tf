...
locals {
  aws_eks_elb_controller_role_name = "${local.environment}-AmazonEKSLoadBalancerController"
  aws_eks_external_dns_role_name   = "${local.environment}-AmazonEKSRoute53ExternalDNS"

  k8s_domains = {
    app  = "${local.environment}.${var.domain}"
    argo = "${local.environment}-argocd.${var.domain}"
  }

  oidc_assume_role_policies = {
    for sa_key, sa_fqn in {
        ...
        Statement = [{
          ...
          Condition = {
            StringEquals = {
              ...
              "${module.eks.oidc_provider}:sub" = sa_fqn
            }
          }
        }]
    })
...
resource "aws_iam_role" "AmazonEKSLoadBalancerControllerRole" {
  ...
  assume_role_policy = local.oidc_assume_role_policies["alb_controller"]
  tags               = var.tags
}

resource "aws_iam_role" "external_dns" {
  name               = local.aws_eks_external_dns_role_name
  assume_role_policy = local.oidc_assume_role_policies["external_dns"]
  tags               = var.tags
}
