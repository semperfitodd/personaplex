...
function initDecoder() {
  ...
  setTimeout(() => {
    if (decoderWorker) decoderWorker.postMessage({ command: 'decode', pages: createWarmupBosPage() });
  }, 100);
...
function buildWSUrl() {
  ...
  if (voiceSelect.value) url.searchParams.set('voice_prompt', voiceSelect.value);
...
const VOICE_GROUPS = [
  { label: 'Natural Female', pattern: /^NATF\d+\.pt$/i },
  { label: 'Natural Male',   pattern: /^NATM\d+\.pt$/i },
  { label: 'Variety Female', pattern: /^VARF\d+\.pt$/i },
  { label: 'Variety Male',   pattern: /^VARM\d+\.pt$/i },
  { label: 'Custom',         pattern: /^voice_prompt_/i },
];

function voiceLabel(filename) {
  return filename.replace(/\.(pt|wav)$/i, '').replace(/_/g, ' ');
}
...
async function loadVoices() {
  ...
  try {
    ...
    for (const v of voices) {
      const group = VOICE_GROUPS.find((g) => g.pattern.test(v));
      if (group) {
        (buckets[group.label] = buckets[group.label] || []).push(v);
      } else {
        ungrouped.push(v);
      }
    }
    ...
    const orderedLabels = VOICE_GROUPS.map((g) => g.label).filter((l) => buckets[l]);
    if (ungrouped.length) buckets['Other'] = ungrouped;
    if (ungrouped.length) orderedLabels.push('Other');

    let firstOption = null;
    for (const label of orderedLabels) {
      const group = document.createElement('optgroup');
      group.label = label;
      for (const v of buckets[label]) {
        const opt = document.createElement('option');
        opt.value = v;
        opt.textContent = voiceLabel(v);
        group.appendChild(opt);
        firstOption = firstOption || opt;
      }
